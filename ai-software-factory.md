...
human/agent review passes. It maps the existing 12-phase `.ai/workflows/feature-delivery.md`
...
The existing 12 phases collapse into stages, each with an **exit gate** (the
...
- **One cross-cutting host** (`dataalgebra-engineering/.github` or a small `roadmap` repo): cross-repo **Epics** + **Briefs**, the **`.ai/` method + templates**, and the org **Project**. This is also where the otherwise-unbacked process workspace gets a home.
